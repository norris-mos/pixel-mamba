...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(sys.path[0]), '../src/pixel/data/rendering')))
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(sys.path[0]), '../src/pixel/data/models/auto')))
...
from utils.modeling import truncate_decoder_pos_embeddings
from utils.transforms import get_transforms
from utils import resize_model_embeddings
...
def main(args: argparse.Namespace):
    ...
    model = PIXBAForPreTraining.from_pretrained(args.model_name_or_path, config=config, **config_kwargs).to(device)
    ...
    logger.info("Running PIXBA masked autoencoding with pixel reconstruction")
    ...
    inputs = {"pixel_values": img.float().to(device)}
    ...
    if mask is not None:
        ...
        inputs.update({"patch_mask": mask.to(device)})
    else:
